...
def mod_get_frequency(period):
	if period>0:
		return 7093789/(period*2)
	else:
		return 0

def mod_get_period_note(period): # returns the note value
	note=-1
	found=False
	for period_set in mod_periods:
		if period in period_set and not found:
			note=period_set.index(period)
			found=True
	return note

def mod_get_finetune_period(period,finetune):
	return mod_periods[finetune][mod_get_period_note(period)]

def mod_get_closest_period(period,finetune):
	differences=[]
	for period_2 in mod_periods[finetune]:
		differences.append(abs(period-period_2))
	return mod_periods[finetune][differences.index(min(differences))]

def get_panned_bytes(byte,pan): # expects an unsigned byte between 0 and 65535. pan value is between -1 and 1 (left and right)
...
for a in range(0,len(mod_sine_table)): # the protracker sine table is only half a wave, so we're completing it here
	mod_sine_table.append(0-mod_sine_table[a])
...
parser.add_argument("play_mode", type=str,help="Selects a different play mode: " + ", ".join(play_modes))
...
mod_tempo=125
mod_ticks=6
mod_ms_per_tick=mod_get_tempo_length(mod_tempo)
mod_ticks_counter=0
...
if mod_channels==0:
...
elif sample_rate<1000 or sample_rate>380000:
	print("Error: Sample rate must be between 1000 and 380000!")
elif not play_mode in play_modes:
	print("Error: Invalid play mode: " + play_mode + ". Accepted modes: " + ", ".join(play_modes))
elif buffer_size<0 or buffer_size>8192:
	...
	if play_mode=="info":
		...
		for a in range(0,len(mod_periods[0])):
		...
		if render_file==None:
			...
			if stereo:
				channels+=1
			stream=pya.open(format=pyaudio.paInt16,rate=sample_rate,output=True,channels=channels,frames_per_buffer=buffer_size)

		if not render_file and not verbose:
			print("Playing...")
		...
		while while_condition:
			...
			mod_period_amount=len(mod_periods[0])
			...
			for a in range(0,mod_channels):
				if play_mode.startswith("stereo_soft"):
					if a%4==1 or a%4==2:
			...
			while mod_order_position<mod_song_length:
				while mod_line<mod_lines:
					...
					pattern_string=""
					loop_string=""
					if loops>1 or render_channels:
						loop_string=" ("
					if loops>1:
						loop_string+="loop " + str(mod_loops+1) + "/" + str(loops)
						if render_channels:
							loop_string+=", "
					if render_channels:
					...
					for channel in range(0,mod_channels):
						...
						if mod_effect_number[channel]==0x3:
							if period>0:
								if mod_tone_period[channel]>0:
									mod_tone_sliding[channel]=True
								mod_tone_period[channel]=mod_get_finetune_period(period,mod_finetune_temp[channel])
						...
						if sample_number>0: # finetune check...
							...
							mod_bpm=(sample_rate/mod_samples_per_beat)*60
						...
						if mod_effect_number[channel]==0xe: # extended effects
							...
							if effect==0xd: # note delay
								if param>=mod_ticks-1: # if the delay amount is the same as the ticks per line, the note is ignored
						...
						if period>0 and not mod_tone_sliding[channel] and mod_pattern_delay_finished: # the period>0 fixes a bug related to pattern delays, if there's a period on the last channel, the period value will contain that, so without the check all channels will have the same period!
							mod_period[channel]=mod_get_finetune_period(period,mod_finetune_temp[channel])
						...
						if mod_effect_number[channel]==0x0: # arpeggio
							if mod_effect_param[channel]>0: # 0 means ignore
								period_note=mod_get_period_note(mod_arp_period[channel])
								if sample_number>0:
									sample_finetune=0

								if mod_effect_param[channel]>>4==0:
									mod_arp_periods[channel][1]=mod_periods[sample_finetune][period_note]
								else:
									...
									if period_1>mod_period_amount-1:
										...
										mod_arp_periods[channel][1]=mod_periods[sample_finetune][period_1]

								if mod_effect_param[channel] & 0xf==0:
									mod_arp_periods[channel][2]=mod_periods[sample_finetune][period_note]
								else:
									...
									if period_2>mod_period_amount-1:
										...
										mod_arp_periods[channel][2]=mod_periods[sample_finetune][period_2]
								mod_arp_periods[channel][0]=mod_periods[sample_finetune][period_note]
							else:
						...
						if mod_arp_periods[channel]!=[0,0,0]:
							for a in range(0,3):
								if mod_arp_periods[channel][a]>0: # if the period is 0, it'll be missed entirely (in mod_get_frequency)
									mod_arp_periods[channel][a]=mod_get_finetune_period(mod_arp_periods[channel][a],mod_finetune_temp[channel])

						if render_file==None:
							if verbose:
								note_name="---"
								note_number=mod_get_period_note(mod_raw_period[channel])
								if note_number>=0:
					...
					if render_file==None:
						if verbose:
							...
							print("Time elapsed: " + time_elapsed_string + ", Tempo: " + str(mod_tempo) + ", Ticks/Line: " + str(mod_ticks) + ", BPM: " + "%g" % mod_bpm + ", Order " + str(mod_order_position) + "/" + str(mod_song_length-1) + ", Pattern " + str(mod_order[mod_order_position]) + ", Line " + str(mod_line+1) + "  ",end="\r")
					...
					while mod_ticks_counter<mod_ms_per_tick*mod_ticks:
						...
						for channel in range(0,mod_channels):
							if mod_ticks_counter_actual_previous!=mod_ticks_counter_actual or mod_ticks_counter==0: # on every tick (including the first)
								...
								if mod_period[channel]>0:
									if mod_glissando[channel]:
										mod_frequency[channel]=mod_get_frequency(mod_get_closest_period(mod_period[channel],mod_samples[sample_number]["finetune"]))
									else:
										if mod_arp_periods[channel]!=[0,0,0]:
											if mod_arp_periods[channel][mod_arp_counter[channel]]>0:
												mod_frequency[channel]=mod_get_frequency(mod_arp_periods[channel][mod_arp_counter[channel]])
											else:
												mod_frequency[channel]=0
										else:
											mod_frequency[channel]=mod_get_frequency(mod_period[channel]+mod_vibrato_offset[channel])
								if mod_arp_periods[channel]!=[0,0,0]:
									if mod_ticks==2:
							...
							if mod_ticks_counter_actual_previous!=mod_ticks_counter_actual: # a tick has occured
								...
								if mod_vibrato[channel] or mod_tremolo[channel]:
									...
									depth=memory & 0xf
									if wave_type==0: # sine
										offset=(mod_sine_table[counter]*depth)/128
									...
									if mod_vibrato[channel]:
										...
										mod_vibrato_counter[channel]=mod_vibrato_counter[channel]%len(mod_sine_table)
									else:
										mod_tremolo_offset[channel]=offset
										mod_tremolo_counter[channel]+=memory>>4
										mod_tremolo_counter[channel]=mod_tremolo_counter[channel]%len(mod_sine_table)
							...
							sample_step_rate=mod_frequency[channel]/sample_rate
						...
						for counter,channel_byte in enumerate(mod_channel_byte):
							if stereo:
								channel_byte_panned=get_panned_bytes(channel_byte,mod_channel_pan[counter])
				...
				if mod_song_length>1:
					...
					mod_loops+=1
				if mod_loops>loops-1: # copypasta SSSSHHHHH (but this is for when the pattern ends, not per line... so if you're line breaking/position breaking this won't be reached)
					mod_order_position=mod_song_length # end
					mod_line=mod_lines
				mod_pointer=mod_pattern_offsets[mod_order[mod_order_position]]

			if render_file and render_channels:
				file_name=".".join(render_file.name.split(".")[:-1])[:-2]+"_"+str(channel_current+1)+"."+render_file.name.split(".")[-1]
		...
		if render_file:
			if not render_channels:
				with wave.open(render_file.name,"w") as wave_file:
					if stereo:
